...
#else //if NO_MILLIS is set, need no millis, micros, and different delay
  ...
  */
      ...
      // it to be set to run at 16, in which case we need to reload the factory cal.
        ...
        OSCCAL = read_factory_calibration(); //we do this if it was tuned by micronucleus, but we don't want USB.
            ...
            #error "Custom tuning is not supported in the current version of ATTinyCore"#endif // end check for each prescale freq
        #endif //end if not 16
